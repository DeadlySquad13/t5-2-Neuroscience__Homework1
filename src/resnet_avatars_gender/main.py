...
def get_value_in_range(v: int, X: Matrix, p: Possibilities):
    border = 0
...
def make_step(current_state: int, transition_matrix: TransitionMatrix):
    ...
    return get_value_in_range(
        np.random.uniform(), possible_states, probabilities_for_current_state
    )
...
def simulate_walks(
    ...
    return [
        walk(initial_state, times_per_walk, transition_matrix)
